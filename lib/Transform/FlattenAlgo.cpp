...
isl::UnionMap tryFlattenSequence(isl::UnionMap Schedule) {
  ...
  auto AllDomainsToNull = isl::UnionPwMultiAff(AllDomains);

  auto NewSchedule = isl::UnionMap::empty(ParamSpace);
  auto Counter = isl::PwAff(isl::LocalSpace(ParamSpace.setFromParams()));
  ...
  while (!ScatterSet.isEmpty()) {
    ...
    auto One = isl::PwAff(isl::Set::universe(ParamSpace.setFromParams()),
                          isl::Val::one(IslCtx));
