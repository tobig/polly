...
isl::MultiAff makeShiftDimAff(isl::Space Space, int Pos, int Amount) {
...
isl::BasicMap makeTupleSwapBasicMap(isl::Space FromSpace1,
...
isl::UnionMap polly::betweenScatter(isl::UnionMap From, isl::UnionMap To,
...
isl::Map polly::singleton(isl::UnionMap UMap, isl::Space ExpectedSpace) {
  if (!UMap)
    return nullptr;
...
isl::Set polly::singleton(isl::UnionSet USet, isl::Space ExpectedSpace) {
  if (!USet)
    return nullptr;
...
isl::Space polly::getScatterSpace(const isl::UnionMap &Schedule) {
  if (!Schedule)
    return nullptr;
...
void polly::simplify(isl::UnionSet &USet) {
...
void polly::simplify(isl::UnionMap &UMap) {
...
isl::UnionMap polly::computeReachingWrite(isl::UnionMap Schedule,
                                          isl::UnionMap Writes, bool Reverse,
...
isl::UnionMap
polly::computeArrayUnused(isl::UnionMap Schedule, isl::UnionMap Writes,
...
isl::UnionSet polly::convertZoneToTimepoints(isl::UnionSet Zone, bool InclStart,
                                             bool InclEnd) {
