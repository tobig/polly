...
void polly::foreachElt(const isl::Map &Map,
                       const std::function<void(isl::BasicMap)> &F) {
  isl_map_foreach_basic_map(
      Map.keep(),
      [](__isl_take isl_basic_map *BMap, void *User) -> isl_stat {
        auto &F =
            *static_cast<const std::function<void(isl::BasicMap)> *>(User);
        F(give(BMap));
        return isl_stat_ok;
      },
...
void polly::foreachElt(const isl::Set &Set,
                       const std::function<void(isl::BasicSet)> &F) {
  isl_set_foreach_basic_set(
      Set.keep(),
      [](__isl_take isl_basic_set *BSet, void *User) -> isl_stat {
        auto &F =
            *static_cast<const std::function<void(isl::BasicSet)> *>(User);
        F(give(BSet));
        return isl_stat_ok;
      },
      const_cast<void *>(static_cast<const void *>(&F)));
...
void polly::foreachElt(const isl::UnionMap &UMap,
...
void polly::foreachElt(const isl::UnionSet &USet,
...
void polly::foreachElt(const isl::UnionPwAff &UPwAff,
                       const std::function<void(isl::PwAff)> &F) {
  isl_union_pw_aff_foreach_pw_aff(
      UPwAff.keep(),
      [](__isl_take isl_pw_aff *PwAff, void *User) -> isl_stat {
        auto &F = *static_cast<const std::function<void(isl::PwAff)> *>(User);
        F(give(PwAff));
        return isl_stat_ok;
      },
      const_cast<void *>(static_cast<const void *>(&F)));
...
polly::foreachEltWithBreak(const isl::Map &Map,
                           const std::function<isl_stat(isl::BasicMap)> &F) {
  return isl_map_foreach_basic_map(
      Map.keep(),
      [](__isl_take isl_basic_map *BMap, void *User) -> isl_stat {
        auto &F =
            *static_cast<const std::function<isl_stat(isl::BasicMap)> *>(User);
        return F(give(BMap));
      },
      const_cast<void *>(static_cast<const void *>(&F)));
}
...
isl_stat
polly::foreachEltWithBreak(const isl::UnionMap &UMap,
