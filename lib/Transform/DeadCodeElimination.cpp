...
private:
  ...
  ///
  isl::UnionSet getLiveOut(Scop &S);
...
isl::UnionSet DeadCodeElim::getLiveOut(Scop &S) {
  isl::UnionMap Schedule = isl::manage(S.getSchedule());
  isl::UnionMap MustWrites = isl::manage(S.getMustWrites());
  isl::UnionMap WriteIterations = MustWrites.reverse();
  isl::UnionMap WriteTimes = WriteIterations.applyRange(Schedule);

  isl::UnionMap LastWriteTimes = WriteTimes.lexmax();
  isl::UnionMap LastWriteIterations =
      LastWriteTimes.applyRange(Schedule.reverse());

  isl::UnionSet Live = LastWriteIterations.range();
  isl::UnionMap MayWrites = isl::manage(S.getMayWrites());
...
bool DeadCodeElim::eliminateDeadCode(Scop &S, int PreciseSteps) {
  ...
  isl::UnionSet Live = getLiveOut(S);
  isl::UnionMap Dep = isl::manage(
  ...
  isl::UnionSet OriginalDomain = isl::manage(S.getDomains());
  int Steps = 0;
  while (true) {
    Steps++;

    isl::UnionSet Extra = Live.apply(Dep);
