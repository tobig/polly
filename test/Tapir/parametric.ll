...
; RUN: -polly-tapiropt -polly-on-isl-error-abort=false \
; RUN: -polly-tapiropt-parametric -polly-parallel < %s | FileCheck %s
;
;    #define N 1024
;
;    void add(float A[][N]) {
...
; CHECK: // Tapir Spawn:
; CHECK: // LB: dim 0 : { val: 0 }
; CHECK: // UB: dim 0 : { val: 1023 }
; CHECK: // Tapir Spawn:
; CHECK: // LB: dim 1 : { val: 0 }

; CHECK: // UB: dim 1 : { val: 1023 }

; CHECK:     #pragma omp parallel for
; CHECK-NEXT:     for (int c0 = Start0; c0 <= min(1023, Start0 + Size0); c0 += 1)
; CHECK-NEXT:       #pragma simd
; CHECK-NEXT:       for (int c1 = Start1; c1 <= min(1023, Start1 + Size1); c1 += 1)
; CHECK-NEXT:         Stmt_for_body4(c0, c1);
